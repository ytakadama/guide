<template>
  <div>
    ...
    <div class="form-frame" v-if="!isInput && !error">
      <div class="form-main">
        <div id="select-button-frame">
          <button
            class="button-green"
            :class="buttonGray(is_possible !== null && is_possible !== 1)"
            @click="is_possible = 1"
          >
          ...
          <button
            class="button-red"
            :class="buttonGray(is_possible !== null && is_possible !== 0)"
            @click="is_possible = 0"
          >
          ...
          <button
            class="button-orange"
            :class="buttonGray(is_possible !== null && is_possible !== 2)"
            @click="is_possible = 2"
          >
            調整すれば可能
          </button>
    ...
    <div v-if="isInput">
      ...
      <p>
        {{ $t("label.send_info") }}
        <span v-if="is_possible === 1">{{
          $t("pages.guides.schedules.button_participate")
        }}</span>
        <span v-if="is_possible === 0">
          {{ $t("pages.guides.schedules.button_absent") }}
        </span>
        <span v-if="is_possible === 2"> 調整すれば可能 </span>
      </p>
    </div>
  </div>
</template>
