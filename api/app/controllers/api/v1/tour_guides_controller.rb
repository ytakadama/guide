    ...
    ApplicationRecord.transaction do
      ...
      if send_mail == true
        ...
        new_assign.each do |g|
          ...
          # RemindNotifyMailer.creation_email(Guide.find(g.guide_id), tour).deliver_now
        end
        ...
        # 担当にならなかった場合に送信
        if is_first_assign
          no_assign_guides = GuideSchedule.where(tour_id: tour_id).where.not(guide_id: guides).where(answered: true)
