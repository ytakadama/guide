...
class Api::V1::GuideSchedulesController < ApplicationController
  ...
  def update
    ...
    if guide.is_invalid == true || (!tour.schedule_input_deadline.nil? && tour.schedule_input_deadline < (DateTime.now - 1))
      render json: json_render_v1(false)
      return
    end
    ...
    # guide_schedule.update(answered: true, possible: 2)
  ...
  def index
    ...
    if tour.start_datetime < Date.today || tour.tour_state_code != TOUR_STATE_CODE_INCOMPLETE || (!tour.schedule_input_deadline.nil? && tour.schedule_input_deadline < (DateTime.now - 1))
      puts("aaaaaaaaaaaaaaaaaaaaaa2")
      render json: json_render_v1(false)
      return
    end
    puts("★★★★★★★★★★★★3")
